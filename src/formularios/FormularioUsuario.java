...
public class FormularioUsuario extends JDialog {
    ...
    public FormularioUsuario(Window parent, UsuariosDAO usuarioDAO, TipoIdentificacionDAO tipoIdentificacionDAO, RolDAO rolDAO, JPanel panelPadreRef) {
    ...
    public FormularioUsuario(Window parent, UsuariosDAO usuarioDAO, TipoIdentificacionDAO tipoIdentificacionDAO, RolDAO rolDAO, JPanel panelPadreRef, Usuario usuarioParaEditar) {
        ...
        try (Connection conn = ConnectionADMIN.getConnectionADMIN()) {
            if (esEdicion) {
                ...
                if (modificado) {
                    ...
                    if (panelPadreRef instanceof paneCuentaAdmin) {
                        ((paneCuentaAdmin) panelPadreRef).cargarDatosUsuario(usuario); 
                    }
                    // 2. Si tienes un panel de cuenta de administrador y quieres que se refresque:
                    else if (panelPadreRef instanceof paneCuentaAdmin) { 
                        ((paneCuentaAdmin) panelPadreRef).cargarDatosUsuario(usuario);
                    }
